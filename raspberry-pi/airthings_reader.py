...
class Wave2:
    ...
    Klassen har ansvar for:
    - Scanning efter sensoren
    - Oprettelse af BLE forbindelse
    - Læsning af målinger
    - Lukning af forbindelsen
    """

    CURR_VAL_UUID = btle.UUID(
        "b42e4dcc-ade7-11e4-89d3-123b93f75cba"
    )
    ...
    def discover(self):
        """
        Scanner efter Airthings sensoren.

        Sensoren identificeres ud fra serienummer.
        """
        ...
        for _ in range(int(timeout / scan_interval)):
            ...
            for adv in advertisements:
                ...
                if self.serial_number == found_serial:
                    ...
                    print(
                        f"Found Airthings device: {adv.addr}"
                    )
    ...
    def connect(self, retries=3):
        ...
        while tries < retries and not self.is_connected():
            ...
            if self.mac_addr is None:
                raise Exception(
                    "Airthings device not found"
                )

            try:

                print(
                    f"Connecting to {self.mac_addr}..."
                )

                self._periph = btle.Peripheral(
                    self.mac_addr
                )
    ...
    def read(self):
        """
        Læser aktuelle målinger fra sensoren.
        """

        rawdata = self._char.read()

        return CurrentValues.from_bytes(
            rawdata
        )
